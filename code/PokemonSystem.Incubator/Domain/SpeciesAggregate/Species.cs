...
{
    public class Species : Entity<uint>, IAggregateRoot
    {
        private const double MIN_MALE_FACTOR = 0;
        private const double MAX_MALE_FACTOR = 1;

        public Species(uint id, string name, Typing typing, Stats baseStats, double? maleFactor, List<EvolutionCriteria> evolutionCriterias, List<MoveByLevel> moveSet) : base(id)
        {
